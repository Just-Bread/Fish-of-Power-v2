...
func jump(player: CharacterBody2D, delta: float) -> void:
	...
	if not Input.is_action_pressed("jump") and is_jumping:
		is_jumping = false
		# stop your jump when released jump button
		# strength is dependent on how long the button was held for
		var multiplier = 0.5 / ($JumpTimer.time_left/ $JumpTimer.wait_time)
